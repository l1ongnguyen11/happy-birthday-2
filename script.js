...
class BirthdayWebsite {
    ...
    animateCountdownHearts() {
        ...
        const drawHeart = (h) => {
            if (!h.element) {
                ...
                el.style.filter = 'drop-shadow(0 2px 2px rgba(0,0,0,0.1))';
        ...
        const tick = () => {
            if (!running) return;
            if (Math.random() < 0.18 && hearts.length < 70) {
    ...
    createHeartMatrix() {
        ...
        // Create 17 columns of falling hearts for full screen coverage
        for (let col = 0; col < 17; col++) {
            const column = document.createElement('div');
            column.className = 'heart-column';
            
            // Create 20 hearts per column for better density
            for (let heart = 0; heart < 20; heart++) {
